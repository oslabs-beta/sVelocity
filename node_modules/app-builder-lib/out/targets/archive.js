...
async function tar(compression, format, outFile, dirToArchive, isMacApp, tempDirManager) {
    ...
    await Promise.all([
        builder_util_1.exec(_7zip_bin_1.path7za, tarArgs, { cwd: path.dirname(dirToArchive) }),
        // remove file before - 7z doesn't overwrite file, but update
        fs_1.unlinkIfExists(outFile),
    ]);
    if (!isMacApp) {
        await builder_util_1.exec(_7zip_bin_1.path7za, ["rn", tarFile, path.basename(dirToArchive), path.basename(outFile, `.${format}`)]);
    }
    if (format === "tar.lz") {
